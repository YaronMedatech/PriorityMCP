...
| The cloud web UI authenticates with **OIDC** (`<host>/auth/`, client `priority-web`, scope `openid wcf_api`) and calls WCF with `Authorization: Bearer <access_token>`; the OIDC server offers `authorization_code`, `client_credentials` and `refresh_token` grants — **no password grant**. Basic user/password on OData answers 401 for a valid web user. | A headless server needs an OAuth client (client_credentials, scope `wcf_api`) or a one-time browser login kept alive by a refresh token. The SDK supports this: `login({ accessToken, authAudience: 'wcf_api', … })` sends `oidc_jwt` instead of a password. Not implemented yet — see `PRIORITY_HOST_URL` in `.env.example`. |
